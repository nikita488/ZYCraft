...
import com.nikita488.zycraft.block.*;
...
public class ModBlocks
{
    ...
    public static final BlockEntry<QuartzCrystalBlock> QUARTZ_CRYSTAL = REGISTRY.object("quartz_crystal")
            ...
            .blockstate((ctx, provider) -> provider.directionalBlock(ctx.getEntry(), state ->
            {
                ImmutableList<ModelFile> models = ImmutableList.of(
                        provider.models().getExistingFile(provider.modLoc("block/quartz_crystal_1")),
                        provider.models().getExistingFile(provider.modLoc("block/quartz_crystal_2")),
                        provider.models().getExistingFile(provider.modLoc("block/quartz_crystal_3")),
                        provider.models().getExistingFile(provider.modLoc("block/quartz_crystal_4")),
                        provider.models().getExistingFile(provider.modLoc("block/quartz_crystal_5")));

                return models.get(state.get(QuartzCrystalBlock.AMOUNT) - 1);
            }))
            .item()
...
/*    public static final BlockEntry<ColorableWallTorchBlock> ZYCHORIUM_WALL_TORCH = REGISTRY.object("zychorium_wall_torch")
            .block(Material.MISCELLANEOUS, ColorableWallTorchBlock::new)
            .properties(properties -> properties.doesNotBlockMovement().hardnessAndResistance(0).lightValue(14).sound(SoundType.WOOD))
            .blockstate((ctx, provider) -> provider.horizontalBlock(ctx.getEntry(), provider.models()
                    .withExistingParent(ctx.getName(), provider.modLoc("block/colorable_wall_torch"))
                    .texture("base", provider.modLoc("block/zychorium_torch_base")), 90))
            .register();

    public static final BlockEntry<ColorableTorchBlock> ZYCHORIUM_TORCH = REGISTRY.object("zychorium_torch")
            .block(Material.MISCELLANEOUS, ColorableTorchBlock::new)
            .properties(properties -> properties.doesNotBlockMovement().hardnessAndResistance(0).lightValue(14).sound(SoundType.WOOD))
            .blockstate((ctx, provider) -> provider.simpleBlock(ctx.getEntry(), provider.models()
                    .withExistingParent(ctx.getName(), provider.modLoc("block/colorable_torch"))
                    .texture("base", provider.modLoc("block/zychorium_torch_base"))))
            .item((block, properties) -> new WallOrFloorItem(block, ZYCHORIUM_WALL_TORCH.get(), properties)).build()
            .register();

    public static final BlockEntry<ColorableWallTorchBlock> ALUMINIZED_ZYCHORIUM_WALL_TORCH = REGISTRY.object("aluminized_zychorium_wall_torch")
            .block(Material.MISCELLANEOUS, ColorableWallTorchBlock::new)
            .properties(properties -> properties.doesNotBlockMovement().hardnessAndResistance(0).lightValue(14).sound(SoundType.WOOD))
            .blockstate((ctx, provider) -> provider.horizontalBlock(ctx.getEntry(), provider.models()
                    .withExistingParent(ctx.getName(), provider.modLoc("block/colorable_wall_torch"))
                    .texture("base", provider.modLoc("block/aluminized_zychorium_torch_base")), 90))
            .register();

    public static final BlockEntry<ColorableTorchBlock> ALUMINIZED_ZYCHORIUM_TORCH = REGISTRY.object("aluminized_zychorium_torch")
            .block(Material.MISCELLANEOUS, ColorableTorchBlock::new)
            .properties(properties -> properties.doesNotBlockMovement().hardnessAndResistance(0).lightValue(14).sound(SoundType.WOOD))
            .blockstate((ctx, provider) -> provider.simpleBlock(ctx.getEntry(), provider.models()
                    .withExistingParent(ctx.getName(), provider.modLoc("block/colorable_torch"))
                    .texture("base", provider.modLoc("block/aluminized_zychorium_torch_base"))))
            .item((block, properties) -> new WallOrFloorItem(block, ZYCHORIUM_WALL_TORCH.get(), properties)).build()
            .register();*/
