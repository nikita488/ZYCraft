...
public class ZychoriteVeinConfig implements IFeatureConfig
{
    ...
    {
        return instance.group(RuleTest.field_237127_c_.fieldOf("target").forGetter((config) ->
        {
            return config.target;
        }), Codec.intRange(0, 64).fieldOf("size").forGetter((config) ->
        {
            return config.size;
        }), Codec.floatRange(0.0F, 1.0F).fieldOf("ore_percentage").forGetter((config) ->
        {
            return config.orePercentage;
